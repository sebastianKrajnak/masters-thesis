...
<template>
  <ModalWindow
    @toggle-modal="updateIntro($event)"
    :visible="sStore.introVisible"
    :key="reloadIntroModal"
  >
    <template v-slot:header> Short How-To </template>
    <template v-slot:content>
      <iframe
        class="intro"
        src="https://www.youtube.com/embed/9EpRY_661Ao"
...
setInterval(() => {
  var [table, queue] = qStore.saveTableQueueToServer();
  socket.emit("save-card-data", { table, queue });
  console.log("Saving...");
}, 7000);
...
onBeforeMount(() => {
  ...
  socket.once("load-card-data", (data) => {
    ...
    console.log(`Data from DB table: ${data.table}, queue: ${data.queue}`);
    qStore.loadTableQueueFromServer(data.table, data.queue);
  });
});
</script>
